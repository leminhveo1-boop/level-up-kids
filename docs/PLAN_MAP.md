...
## 📍 ĐANG Ở ĐÂU
Xong **B2 + B3** (chẩn đoán→kê lộ trình, thẻ Khoảnh khắc) + **PROD-1 (cân liều thưởng: fade-out + UI badge) — live + verify, deploy `c8e31be6`**. Đang **chờ phản hồi 10 thẻ từ khách** trước khi làm sâu tiếp. B4/B5 chưa khởi công (PROD-1 đã mở khoá "sensor sạch" cho B4).
...
|---|---|---|---|---|---|
| **PROD-1** | Cân liều thưởng — tránh dạy con "làm vì thưởng" + không nhiễu sensor Đọc vị | ✅ **live + verify** (deploy `c8e31be6`) | 🔴spec→🔴impl→🔴verify | **Cao — trước B4** | rà cơ chế điểm/thưởng; đặt trần/tần suất; test không đảo ngược động lực nội tại (thẻ #10 + DEEP #3 + report persona). **Xong:** [SPEC](SPEC_PROD1_CAN_LIEU_THUONG.md) fade-out ease-in theo `habitStreak` (FADE_START=14, FADE_FLOOR=0.6, KHÔNG trần cứng); `rewardDoseFactor`+`completeTask`; `reward-dose.test.js` 12 ca, **220/220 vitest + build xanh**; UI badge "Còn N ngày thành Bản Năng" (INV-5, verify /demo: điểm dose khớp + nâu ấm #5C5243). INV-7: deploy không tụt điểm ai |
