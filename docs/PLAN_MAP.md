...
Claude Code (Opus/Fable) chỉ nên tiêu token ở **2 đầu có phán xét cao**: (a) viết SPEC + DoD + khung test, (b) VERIFY tính đúng/an toàn trước deploy. **Phần cơ học ở giữa** (sinh nội dung theo mẫu, wiring UI, i18n, tag, boilerplate, sửa lặp) → đẩy sang **Antigravity IDE/CLI** (quota riêng, không tốn token Claude) hoặc **Haiku** (tier rẻ trong Claude Code).
...
| 🔴 **O/F** | Opus 4.8 / Fable 5 | Viết SPEC, kiến trúc, **audit an toàn/bảo mật**, thiết kế GIỌNG nội dung, VERIFY logic nhạy cảm | Đắt (Claude) |
...
|---|---|---|---|
| 1 | **PROD-1** design luật cân liều thưởng | 🔴 O/F | đụng kinh tế lõi + sensor Đọc vị — phán xét cao |
| 2 | **PROD-1** implement + test từ spec | 🟡 S | đề bài đã rõ → code thẳng |
| 3 | **PROD-1** verify không đảo động lực nội tại | 🔴 O/F | nhạy cảm, không giao máy rẻ |
...
| 6 | **EXP-3 (B4)** viết SPEC lát Đọc vị + rào an toàn | 🔴 O/F | ràng buộc DEEP #3, stakes cao |
| 7 | **EXP-3 (B4)** implement rule engine + test | 🟡 S / 🔷 AG | spec chặt → Sonnet, hoặc AG nếu quá cơ học |
| 8 | **EXP-3 (B4)** verify an toàn (cấm chẩn đoán) | 🔴 O/F | **bắt buộc Claude**, không giao máy rẻ |
| 9 | **EXP-4 (B5)** SPEC định dạng share | 🔴 O/F | STEPPS + cấm lộ định danh — phán xét |
...
|---|---|---|---|---|---|
| **PROD-1** | Cân liều thưởng — tránh dạy con "làm vì thưởng" + không nhiễu sensor Đọc vị | ⏳ | 🔴spec→🟡impl→🔴verify | **Cao — trước B4** | rà cơ chế điểm/thưởng; đặt trần/tần suất; test không đảo ngược động lực nội tại (thẻ #10 + DEEP #3 + report persona) |
...
|---|---|---|---|---|
| **SEC-1** | Đợt audit bảo mật trước launch: fix 2 CVE `next`/`postcss` (lên Next 16 breaking) + rotate toàn bộ token/secret + rà RLS/webhook | ⏳ (hoãn có chủ đích) | 🔴 O/F (**không giao máy rẻ**) | 1 đợt gộp; KHÔNG nhắc lẻ tới lúc đó (quyết định founder) |
